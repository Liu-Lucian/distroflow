...
try:
    ...
    from smart_email_finder import SmartEmailFinder
...
def run_fully_automated_campaign(
    ...
    # Use auth.json from MarketingMind AI directory
    auth_file = str(SCRIPT_DIR / "auth.json")

    # Use Smart Email Finder (Hunter.io + LLM hybrid)
    finder = SmartEmailFinder(
        auth_file=auth_file,
        enable_email_verification=True,  # Will use Hunter.io verifier
        smtp_timeout=10,
        hunter_api_key='1553249bbb256b2a3d111c9c67755c2927053828'  # Explicitly pass API key
    )
...
Examples:
  ...
  python fully_automated_campaign.py --auto-generate --leads 100 --no-auto-followup
    ...
    product_group.add_argument(
        '--auto-generate',
        action='store_true',
        help='Auto-generate keywords from current project directory'
    )
    product_group.add_argument(
        '--product',
        type=str,
        choices=['hiremeai', 'HireMeAI'],
        help='Use predefined product (hiremeai)'
    )
    product_group.add_argument(
        '--product-file',
        type=str,
        help='Path to custom product description file'
    )
    product_group.add_argument(
        '--list-products',
        action='store_true',
        help='List all available predefined products'
    )
    ...
    try:
        ...
        if args.list_products:
            ...
            if products_dir.exists():
                for product_file in products_dir.glob("*.md"):
                    logger.info(f"  • {product_file.stem}")
                    logger.info(f"    Usage: marketing-campaign --product {product_file.stem} --leads 100")
                    logger.info("")
            else:
                logger.warning("  No predefined products found")
            sys.exit(0)
        ...
        if args.auto_generate:
            ...
            if not os.path.exists(product_file):
                logger.error(f"❌ Predefined product not found: {args.product}")
                logger.info("Run 'marketing-campaign --list-products' to see available products")
                sys.exit(1)
            logger.info(f"✅ Using predefined product: {args.product}")
        else:
